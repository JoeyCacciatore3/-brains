    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          const error = new Error('Invalid API key. Please check your Groq API key configuration.');
          ...
          const error = new Error('Groq service is temporarily unavailable. Please try again later.');
          (error as ErrorWithCode).code = ErrorCode.LLM_PROVIDER_ERROR;
          throw error;
        } else {
      ...
      const parser = new SSEParser('Groq');
      let fullContent = '';
        ...
        while (true) {
          ...
          for (const data of dataLines) {
            ...
            if (parseResult.success && parseResult.data) {
              ...
              logger.warn('Failed to parse JSON chunk from Groq API (invalid JSON)', {
                provider: 'Groq',
      ...
      if (!fullContent || fullContent.trim().length === 0) {
        const error = new Error('Empty response received from Groq API. Please try again.');
        (error as ErrorWithCode).code = ErrorCode.LLM_PROVIDER_ERROR;
      ...
      if (fullContent.trim().length < 10) {
        logger.warn('Suspiciously short response from Groq API', {
          provider: 'Groq',
  ...
  protected async completeThoughtInternal(
    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          logger.warn('Failed to get continuation from Groq API', {
            provider: 'Groq',
